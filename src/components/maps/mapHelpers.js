const makeIcon = (function () {
  ...
  /* The site code, drawn inside the marker body.
     Three things it has to stay distinct from, and no flat colour manages all
     three on its own -- the body underneath is green, red, amber or grey
     depending on the site:
       - the marker's own outline, which is white, so white text merged into it
       - Google's place labels, which are near-black, so black text reads as
         just another map label
       - the terrain, which is shades of green and grey
     A saturated blue is none of those, and the white halo behind it (paint-order
     draws the stroke first) lifts it off whichever marker colour it lands on. */
  const nameTemplate = processTemplate('<text x="50%" y="35%" font-size="6" font-weight="bold" font-family="Verdana" fill="#1552d0" stroke="white" stroke-width="1.4" stroke-linejoin="round" paint-order="stroke" text-anchor="middle" dominant-baseline="middle" lenghtAdjust1="spacingAndGlyphs" textLength1="15">`name`</text>')
